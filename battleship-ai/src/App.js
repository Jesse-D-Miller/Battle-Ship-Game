...
function App() {
  ...
  const handlePlayerClick = (r, c) => {
    ...
    const { board: nextAIBoard, ships: nextAIShips, result, sunk } =
    ...
    if (result === 'hit') {
      setLastEvent(sunk ? `You sunk the enemy's ${nextAIShips[sunk].name}!` : 'Hit!');
  ...
  useEffect(() => {
    ...
    const tid = setTimeout(() => {
      ...
      const { board: nextPlayerBoard, ships: nextPlayerShips, result, sunk } =
      ...
      if (result === 'hit') {
        setLastEvent(sunk ? `Enemy sunk your ${nextPlayerShips[sunk].name}!` : 'Enemy hit!');
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <p style={{ marginTop: 16 }}>
        ...
        {gameOver ? `Game Over — ${winner === 'player' ? 'You Win!' : 'You Lose.'}` : `Turn: ${turn}`}
      </p>
      <p>{lastEvent}</p>
    </div>
  );
